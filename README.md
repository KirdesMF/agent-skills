...
```bash
pnpx skills add KirdesMF/agent-skills --skill='*'
```

or to install all of them globally:

```bash
pnpx skills add KirdesMF/agent-skills --skill='*' -g
```
...
This collection is aim to be a one-stop collection of you are mainly working on Vite. It includes skills from different sources with different scopes.
...
This collection is opinionated, but the key difference is that it uses git submodules to directly reference source documentation. This provides more reliable context and allows the skills to stay up-to-date with upstream changes over time. If you primarily work with Vite, this aims to be a comprehensive one-stop collection.
...
4. Run `bun start cleanup` to remove existing submodules and skills
5. Run `bun start init` to clone the submodules
6. Run `bun start sync` to sync vendored skills
